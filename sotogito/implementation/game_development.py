...
while True:
    ...
    if not_move_count == 0:     # note 만약 네 방향 모두 이미 가본 칸이거나 바다로 되어 있는 칸인 경우에는
        back_index = (dir + 1) % 4 #진입한 방향의 정 반대
        nx = x - dx[direction.index(back_index)]  #note 바라보는 방향을 유지한 채로
        ny = y - dy[direction.index(back_index)]

        if is_move(nx, ny): #note 한 칸 뒤로 가고 1단계로 돌아간다.
            x, y = nx, ny
            move_count += 1
            not_move_count = 4
            continue
